...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    // The backing bitmap is sized in device pixels and the context scaled by
    // the same ratio, so shapes stay crisp on retina screens and keep their
    // proportions instead of being stretched from a lower-res buffer.
    const resizeCanvas = () => {
    ...
    // The section can change height after mount (web fonts swapping in reflow
    // the copy), which never fires a window resize. Watching the canvas box
    // keeps the bitmap in sync with whatever size the section settles at.
    ...
    const drawGrid = () => {
      const { w: viewW, h: viewH } = cssSize.current;
      ...
      if (isHex) {
        ...
        const cols = Math.ceil(viewW / hexHoriz) + 3;
        const rows = Math.ceil(viewH / hexVert) + 3;
        ...
        const cols = Math.ceil(viewW / halfW) + 4;
        const rows = Math.ceil(viewH / squareSize) + 4;
        ...
        const cols = Math.ceil(viewW / squareSize) + 3;
        const rows = Math.ceil(viewH / squareSize) + 3;
